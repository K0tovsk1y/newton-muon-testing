...
optimizer2 = Muon(raw_model.transformer.h.parameters(), lr=0.1*args.learning_rate, momentum=0.95,
                  ...
                  # У них scheduler — ТРАПЕЦИЯ (без cosine), поэтому повышать threshold
                  # сильно опасно: на плато идёт ровный lr и нет «естественной» рассинхронизации.
                  # path_threshold=0.013 = 32 × 0.0004 → НА ПЛАТО рефреш каждые 32 шага
                  # (точно как в оригинале), а на warmdown lr падает → рефреш реже.
                  # Итог: ~163 refresh за обучение вместо 194 у авторов (~16% меньше cholesky)
                  # и в среднем ~38 шагов между refresh.
